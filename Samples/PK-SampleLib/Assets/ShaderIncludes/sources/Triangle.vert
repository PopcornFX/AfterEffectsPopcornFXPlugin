...
#	if defined(VRESOURCE_EnabledsOffsets)
	...
	enabled = (e != 0);
#	endif	// defined(VRESOURCE_EnabledsOffsets)
	const uint		storageId = GET_CONSTANT(GPUBillboardPushConstants, StreamOffsetsIndex);
	const vec3		pos0 = LOADF3(GET_RAW_BUFFER(GPUSimData), LOADU(GET_RAW_BUFFER(Positions0Offsets), RAW_BUFFER_INDEX(storageId)) + RAW_BUFFER_INDEX(particleID * 3));
	const vec3		pos1 = LOADF3(GET_RAW_BUFFER(GPUSimData), LOADU(GET_RAW_BUFFER(Positions1Offsets), RAW_BUFFER_INDEX(storageId)) + RAW_BUFFER_INDEX(particleID * 3));
	const vec3		pos2 = LOADF3(GET_RAW_BUFFER(GPUSimData), LOADU(GET_RAW_BUFFER(Positions2Offsets), RAW_BUFFER_INDEX(storageId)) + RAW_BUFFER_INDEX(particleID * 3));
